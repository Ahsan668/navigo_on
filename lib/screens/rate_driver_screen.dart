      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
                onPressed: () {
                  CollectionReference rateDriverRef = FirebaseFirestore.instance.collection("drivers")
                      .doc(widget.assignedDriverId!)
                      .collection("ratings");

                  rateDriverRef.get().then((snap){
                    if(snap == null){
                      rateDriverRef.add(countRatingStars.toString());

                      Navigator.pop(context);
                      Navigator.push(context, MaterialPageRoute(builder: (c) => SplashScreen()));
                    }
                    else {
                      double pastRatings = double.parse(snap.toString());
                      double newAverageRatings = (pastRatings + countRatingStars)/ 2;
                      rateDriverRef.add(newAverageRatings.toString());
